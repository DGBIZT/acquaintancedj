...
def my_view(request):
    ...
    if not data:
        data = 'some expensive computation' # Если данных в кэше не было, выполняем сложные вычисления
...
class CategoryProductsView(View):
    ...
    def get(self, request, category_id: int = None):
        ...
        # Получаем категории из кэша только если кэширование включено
...
class ProductDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        if settings.CACHE_ENABLED:
            ...
            # Если кэширование отключено, получаем объект напрямую
            obj = super().get_object(queryset)
